...
- **Type consistency check:** `flag_file/clear_flag/list_flagged` names used identically in Tasks 2 and 7; `_drain_sort_key` tuple `(rank, requested_at)` used only in Task 3; `extract_page_deep` name identical in Tasks 6 and drain dispatch; `PageClass.VECTOR_DRAWING` spelled identically in Tasks 5-6; config keys `visual_triage_paths`, `deep_scan.*`, `enrichment.*` consistent across Tasks 3-7.
